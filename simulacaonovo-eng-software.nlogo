...
to setup
  ...
  reset-ticks
  set vitorias-devs 0
  set vitorias-langs 0

  create-turtles num-devs [
    setxy random-xcor random-ycor
    set shape "person"
    set color blue
    set tipo "dev"
    set experiencia random dev-experiencia-max + 1
    set agilidade random dev-agilidade-max + 1
    set habilidade random dev-habilidade-max + 1
    set morto? false
    set size 1.5
  ]

  create-turtles num-langs [
    setxy random-xcor random-ycor
    set shape "square"
    set color one-of [orange yellow green]
    set tipo "language"
    set popularidade random language-popularidade-max + 1
    set performance random language-performance-max + 1
    set facilidade random language-facilidade-max + 1
    set morto? false
    set size 1.2
  ]
...
to go
  ...
  ask turtles with [tipo = "dev" and not morto?] [
    ...
    if lang != nobody [
      let lang-performance [performance] of lang
      let lang-facilidade [facilidade] of lang

      let chance-vitoria ((experiencia + habilidade + agilidade) -
                         (lang-performance + (10 - lang-facilidade))) / 10

      ifelse (random-float 1) < (0.5 + chance-vitoria) [
        ask lang [
          set morto? true
  ...
  if languages-count = 0 [
    user-message (word "Desenvolvedores venceram! Restaram: " devs)
    stop
  ]
